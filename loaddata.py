...
#    import os.path
#
#    nbytes = os.path.getsize(Sname)
#
#    if (nbytes == (4*nx*ny*2)): #Two angles in the file	
...
#		Su = np.cos(theta)
#        	return Se,Sn,Su
#	elif (nbytes == (4*nx*ny*3)):   #3 Components of LOS vector in file
#		fin = open(Sname,'rb'); #Open in Binary format
#                phas = np.fromfile(file=fin, dtype=np.float32, count=3*nx*ny)
#		fin.close()
#
#		phs = np.reshape(phs,(ny,3*nx))
#
#		Se = phs[:,::3]
#		Sn = phs[:,1::3]
#		Su = phs[:,2::3]
#        	return Se,Sn,Su
#	else:
#		logger.error('Unknown Format. Geometry file does not have desired dimensions.')
#		sys.exit(1)
#
#
#######End of load_S
